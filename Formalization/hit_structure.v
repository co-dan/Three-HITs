...
Variable I : Type.
Variable C : I -> polynomial.
Variable B : Type. (* this is the indexing type *)
Variable A : Type. (* this is the type that we eliminate into *)
Definition Fconst := fun (_:B) => A. (* the phony type family *)
Variable c : forall i, poly_act (C i) B -> B. (* the constructors *)
Variable f : forall i, poly_act (C i) A -> A. (* the images of the constructors *)
...
Defined. (* this definition is correct *)
...
(*
Definition poly_act_fam (P : polynomial) (F : Type)
  (x : poly_act P F) : 
  { y : poly_act P Unit | poly_fam P (fun (_ : Unit) => F) y }.
Proof.
  induction P; simpl in *. 
  - exists tt. exact x.
  - exists x. exact x.
  - destruct x as [x1 x2].
    destruct (IHP1 x1) as [y1 cow1].
    destruct (IHP2 x2) as [y2 cow2].
    exists (y1,y2). simpl. exact (cow1, cow2).
  - destruct x as [x | x].
    + destruct (IHP1 x) as [y cow].
      exists (inl y). exact cow.
    + destruct (IHP2 x) as [y cow].
      exists (inr y). exact cow.
Defined.

Lemma poly_act_fam_const1 P F x :
  poly_fam_const P F (poly_act_fam P F x) = x.
Proof.
  induction P; auto.
  - destruct x as [x1 x2].
    specialize (IHP1 x1).  specialize (IHP2 x2).
    SearchAbout fst identity.
    apply path_prod.
    + assert (fst (x1,x2) = poly_fam_const P1 F (poly_act_fam P1 F x1)).
      { symmetry. apply IHP1. }
      rewrite X. compute. reflexivity.
    + assert (snd (x1,x2) = poly_fam_const P2 F (poly_act_fam P2 F x2)).
      { symmetry. apply IHP2. }
      rewrite X. reflexivity.
  - simpl in x. destruct x as [x | x].
    + transitivity (@inl _ (poly_act P2 F) (poly_fam_const P1 F (poly_act_fam P1 F x))).
      compute. reflexivity. rewrite (IHP1 x). reflexivity.
    + transitivity (@inr (poly_act P1 F) _ (poly_fam_const P2 F (poly_act_fam P2 F x))).
      compute. reflexivity. rewrite (IHP2 x). reflexivity.
Qed.

Lemma poly_act_fam_const2 P F x :
  poly_act_fam P F (poly_fam_const P F x) = x.
Proof.
  destruct x as [y H].
  induction P; simpl in y; simpl in H; auto.
...
Definition point_in_over (H : HIT) {U : Type}
  (c : point_in U (hit_point H) (hit_path H)) :
  point_over (fun (_ : H) => U) (hit_point H) (hit_path H).
Proof.
  intros i u h. 
  apply (c i).
...
(*
Lemma endpoint_dact_const (H : HIT) (U : Type) {P Q}
  c (x: poly_act P H) (h : poly_fam P (fun _ : H => U) x) 
  (e : endpoint (sig_point Σ) P Q) :
endpoint_dact (hit_point H) (point_in_over H c) e x h 
= endpoint_act c e (poly_fam_const x h).
Proof.
induction (sig_path_lhs Σ j).


Definition path_in_over (H : HIT) {U : Type}
  (c : point_in U (hit_point H) (hit_path H)) 
  (p : path_in U c) :
  path_over (point_in_over H c).
Proof.
  intros j x h. simpl.
  etransitivity. apply transport_const.
  specialize (p j (poly_fam_const j x h)).

  assert (poly_act (sig_path_param Σ j) U) as x'.
  { clear p. 
...
Proof.
  ...
  - specialize (p i).
    ...
    Check (cowd (sig_point_index Σ) (sig_point Σ) H F (hit_point H) c _ _ (sig_path_lhs Σ i) u h).
    assert (poly_fam_const H F poly_var
         (endpoint_act (hit_point H) (sig_path_lhs Σ i) u)
         (endpoint_dact (hit_point H)
            (f' (sig_point_index Σ) (sig_point Σ) H F c)
            (sig_path_lhs Σ i) u h)
    = poly_fam_const H F poly_var
         (endpoint_act (hit_point H) (sig_path_rhs Σ i) u)
         (endpoint_dact (hit_point H)
            (f' (sig_point_index Σ) (sig_point Σ) H F c)
            (sig_path_rhs Σ i) u h)) as COWEQ.
    { etransitivity. symmetry.
      apply (cowd (sig_point_index Σ) (sig_point Σ) H F (hit_point H) c _ _ (sig_path_lhs Σ i) u h).
      rewrite p. 
      apply (cowd (sig_point_index Σ) (sig_point Σ) H F (hit_point H) c _ _ (sig_path_rhs Σ i) u h). }
    Check (poly_fam_const_inj H F poly_var x _ _).
Abort.  
End HIT_Definition.
